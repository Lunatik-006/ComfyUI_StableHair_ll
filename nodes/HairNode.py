...
# Определяем тип устройства (CPU / CUDA)
deviceType = model_management.get_torch_device().type
current_dir = os.path.dirname(os.path.abspath(__file__))
sd_config_dir = os.path.join(current_dir, "libs/configs/sd15")
hair_model_path_format = 'StableHair/{}'
...
class LoadStableHairRemoverModel:
    ...
    def load_model(self, ckpt_name, bald_model, device):
        logger.info(f":: > LoadStableHairRemoverModel started with ckpt={ckpt_name}, bald_model={bald_model}, device={device}")
        ...
        try:
            ...
            device_type = deviceType if device == "AUTO" else "cpu"
            weight_dtype = torch.float16 if device_type == "cuda" else torch.float32

            remove_hair_pipeline = StableDiffusionControlNetPipeline.from_single_file(
                sd15_model_path,
                config=sd_config_dir,
                torch_dtype=weight_dtype,
                local_files_only=True,
            ...
            logger.info(f":: < LoadStableHairRemoverModel completed in {duration:.4f}s")
            return remove_hair_pipeline,

        except Exception as e:
            logger.error(f":: ERROR LoadStableHairRemoverModel: {e}", exc_info=True)
            raise
...
class LoadStableHairTransferModel:
    ...
    def load_model(self, ckpt_name, encoder_model, adapter_model, control_model, device):
        logger.info(f":: > LoadStableHairTransferModel started with ckpt={ckpt_name}")
        start_time = time.perf_counter()
        ...
        try:
            ...
            pipeline = StableHairPipeline.from_single_file(
                sd15_model_path,
                config=sd_config_dir,
                torch_dtype=weight_dtype,
                local_files_only=True,
            ...
            controlnet = ControlNetModel.from_unet(pipeline.unet).to(device_type)
            controlnet.load_state_dict(torch.load(control_model_path), strict=False)
            controlnet.to(device_type, dtype=weight_dtype)
            pipeline.register_modules(controlnet=controlnet)
            ...
            hair_encoder = RefHairUnet.from_config(pipeline.unet.config)
            hair_encoder.load_state_dict(torch.load(encoder_model_path), strict=False)
            hair_encoder.to(device_type, dtype=weight_dtype)
            pipeline.register_modules(reference_encoder=hair_encoder)

            hair_adapter = adapter_injection(pipeline.unet, device=device_type, dtype=weight_dtype, use_resampler=False)
            hair_adapter.load_state_dict(torch.load(adapter_model_path), strict=False)

            duration = time.perf_counter() - start_time
            logger.info(f":: < LoadStableHairTransferModel completed in {duration:.4f}s")
            return pipeline,

        except Exception as e:
            logger.error(f":: ERROR LoadStableHairTransferModel: {e}", exc_info=True)
            raise
